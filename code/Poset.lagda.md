...
SatPosetAxioms : (ℓ₁ : Level) (A : Type ℓ) → RawPosetStr ℓ₁ A → Ω (ℓ ⊔ ℓ₁)
SatPosetAxioms _ A (_⊑_ , A-set) = IsReflexive ∧ IsTransitive ∧ IsAntisym
  where
    open AlgebraicProperties A-set _⊑_

PosetStr : (ℓ₁ : Level) → Type ℓ → Type (ℓ ⊔ suc ℓ₁)
PosetStr ℓ₁ =
  add-to-structure (RawPosetStr ℓ₁) (λ A RP → SatPosetAxioms ℓ₁ A RP is-true)
...
RP-iso-prop : (P Q : Σ (Type ℓ₀) (RawPosetStr ℓ₁))
            → (i : π₀ P ≃ π₀ Q) → IsProp (RP-iso P Q i)
...
poset-iso {ℓ₁ = ℓ₁} =
  add-to-iso (RawPosetStr ℓ₁) RP-iso λ A str → SatPosetAxioms ℓ₁ A str is-true

poset-axioms-props : (A : Type ℓ₀) (str : RawPosetStr ℓ₁ A)
                   → IsProp (SatPosetAxioms ℓ₁ A str is-true)
poset-axioms-props {ℓ₁ = ℓ₁} A str = is-true-prop (SatPosetAxioms ℓ₁ A str)

poset-is-SNS' : SNS' {ℓ = ℓ} (PosetStr ℓ₁) poset-iso
poset-is-SNS' {ℓ₁ = ℓ₁} =
  add-axioms-SNS' _ _
    (λ A str → SatPosetAxioms ℓ₁ A str is-true)
    poset-axioms-props
    raw-poset-is-SNS'
