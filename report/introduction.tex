...
transformations which are expressed through the notion of a topological space. The idea of
continuity pervades practically all of mathematics as pointed out by
Sylvester~\cite[pg.~27]{armstrong-topology}: ``if I were asked to name, in one word, the
pole star round which the mathematical firmament revolves, the central idea which pervades
the whole corpus of mathematical doctrine, I should point to Continuity as contained in
our notions of space, and say, it is this, it is this!''. Let us then start by considering
the question of what topology is, especially from the perspective of computer science.
...
\begin{defn}[Topological space]\label{defn:topospace}
  ...
  \end{itemize}
\end{defn}
When some set $X$ is equipped with such a topology $\mathcal{T}$, it is said to form a
topological space. Its elements are then called \emph{points} and the members of
$\mathcal{T}$ are called \emph{open sets} of $X$.

A noteworthy thing about this definition is the asymmetry between intersection and union:
a topology is required to be closed only under finite intersection whereas it \emph{must}
be closed under arbitrary unions. The same asymmetry is also encountered in computer
science.

Let $P$ be a program whose internal structure we do not have access to. One could say it
is a ``black box'' although this is not to say it is a scenario occurring only in theory;
black boxes \emph{do} occur in practice. A compiled program whose source code one does not
have access to, or even, a program whose source code is not familiar to one, can be seen
as examples of black boxes from one's perspective.
...
be a \emph{finite} prefix of the output of the program by observing which we will able
judge for sure that $P$ satisfies this property. We refer to such properties as
...
\begin{defn}[Observable property (informal)]
  ...
  that an extensional property $\phi$ of $P$ is observable iff
  \begin{center}
    ...
    given any $\sigma'$ that has $\sigma|_i$ as a prefix, $P$ will still satisfy the property even
    if its output is $\sigma'$.
  \end{center}
\end{defn}

Another name for the class of observable properties in this context is ``semidecidable''
although we will prefer the term ``observable'' as we will soon be generalising it to
things that are not programs. Let us now consider some properties of the class of
observable properties themselves.
...
\begin{equation*}
  m_1, \cdots , m_n
\end{equation*}
such that $\phi_k$ is verified at stage $m_k$. $\phi_1 \wedge \cdots \wedge \phi_n$ must then be verified at
$\max(m_1, \cdots, m_n)$. This is to say: if $\phi_1, \cdots, \phi_n$ are \emph{observable} then so is
...
We have given an informal argument that the class of observable properties of programs
forms a topology. This will be the perspective towards topology we will adopt in this
thesis: we will view it as a mathematical theory of observable properties. This
perspective towards topology has been suggested by many researchers. Although it probably
...
bases of finite examinations of a given program. To work with the points directly then is
to trivialise the fact that we do not, in general, have complete understanding of
programs.

A view of topology more compatible with this perspective is what is called
\emph{pointless} topology~\cite{johnstone-the-point}. In pointless topology, points take a
back seat to open sets: instead of starting with a set of points, from which the notion of
an open set is derived, we start with a primitive set of observable properties (called the
``opens''), and we then axiomatise the behaviour of these to reflect that they behave as a
system of observable properties. In other words, we axiomatise the behaviour of the
lattice of open sets of a topological space directly, that is, as an arbitrary lattice
rather that a lattice of subsets.
...
computationally access (i.e.~construct). The precise relationship between these two
approaches is an \emph{adjunction} called \emph{Stone duality}, originating from the
seminal work of Marshall Stone. More precisely, spaces are equivalent to their pointless
representation exactly when they satisfy the property of being sober. We will not go into
this topic in detail; the interested reader is referred to the work of
...
context where we are viewing topology as a theory of observable properties. Does regular
topology gain anything from this pointless approach? Topology notoriously relies on
...
saliently of such important theorems such as the
...
Our goal in this thesis is to investigate formal topology in the context of univalent type
...
in mathematical developments in type theory. The question of what novelties it presents
for formal topology is therefore a natural one. In attempting to answer this question, we
follow a particular approach to formal topology, implementing an idea of
Coquand~\cite{coq-posets} to define formal topologies as posets endowed with
``interaction'' structures~\cite{tree-sets, hancock-interaction-systems}.
