...
explained by Escardó~\cite{escardo-uf-intro}). The first published SIP was formulated by
...
\begin{defn}[Structure identity principle]
...
$\oftyI{r}{x = y}$ of equality to even be able to write this down, but the key idea is
...
\begin{equation*}
  \oftyII{(A, s)}{(B, t)}{\sigmaty{A}{\univ{}}{S(A)}}.
\end{equation*}
Suppose that $(A, s) \simeq_\iota (B, t)$. We already have an equivalence between $A \simeq B$ meaning
we have an inhabitant $p$ of the identity type $A = B$ by univalence. What is then needed
is an inhabitant of the identity type between $s = t$ (modulo transportation by $p$). This
comes from the $\iota$-equivalence between $s$ and $t$ which is ensured by SNS condition (2)
to be strong enough to give us this result. At this point, it might not be clear how a
notion of structure is proven to satisfy (2). We will explain how frames are proven to
form an SNS soon which we hope will clarify this.
...
\begin{equation*}
  \typequiv{(\posetiso{P}{Q})}{\typequiv{(\posetequiv{P}{Q})}{(P = Q)}}.
\end{equation*}
...
We conclude this section by briefly explaining how it is that frames form an SNS. The
...
\begin{proof}
  ...
  $\entire{\abs{P}}$ and $\capp$ (definitions \ref{defn:entire-subset} and
  ...
  \end{align*}
  $\entire{\abs{P}}$ and $\capp$ are propositional by construction whereas $\bigvee$ is not and
  is hence forced to be propositional using truncation. Downwards-closure and the LUB/GLB
  properties are easy to verify. For the distributivity law, let $U$ be a downwards-closed
  subset and $\setof{ V_i ~|~ \oftyI{i}{I} }$, a family of downward-closed subsets. We
  must show
  \begin{align*}
...
\begin{agdanotation}
  The corresponding function in the \veragda{} formalisation is called \fnname{𝔣𝔦𝔵}.
\end{agdanotation}
