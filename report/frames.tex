...
of isomorphism of structures is well-behaved. Such a well-behaved definition of what it
means for an equivalence of carrier types to respect a given structure is called a
...
virtue of preserving the poset structure on types $A$ and $B$. We call this a
\emph{monotonic equivalence} and denote the type of monotonic equivalences between
...
In the case of frames, our notion of homomorphism does not even need to require the
preservation of frame data for it to be an isomorphism (as remarked in
Defn.~\ref{defn:frame-iso-official}); simply preserving the partial order suffices. It is,
however, quite convenient to carry out the SNS proof with a notion of frame homomorphism
that explicitly requires the preservation of frame data. It is for this reason that, in
the \veragda{} formalisation, we first obtain the result (using the SIP)
...
these results that involve frames live in the \modname{Frame} module. The term witnessing
the truth of (\ref{eqn:frame-equiv-1}) is called \fnname{≃f≃\equiv} whilst the one for
(\ref{eqn:frame-equiv-2}) is called \fnname{≃f≃≅ₚ}. The witness of the main result
(\ref{eqn:frame-equiv-3}) is called \fnname{≅ₚ≃≡}.
...
case of a topos, and to \emph{left exact modalities} in the more general case of an
$\infty$-topos.
...
itself a frame, being the quotient frame described by $j$.
...
found in the \modname{Poset} module under the name \fnname{DCPoset}.
...
corresponding \veragda{} proof can be found in the \modname{Frame} module under the name
\fnname{DCFrame}.
...
\begin{proof}
  ...
  $\oftyII{x}{y}{\abs{F}}$ such that $j(x) = x$ and $j(y) = y$. $x \wedge y \sqsubseteq j(x \wedge y)$ by
  $N_1$ so it suffices to show $j(x \wedge y) \sqsubseteq x \wedge y$. By $N_0$, $j(x \wedge y) = j(x) \wedge j(y)$.
  Furthermore, $j(x) \wedge j(y) \sqsubseteq j(x) = x$ and $j(x) \wedge j(y) \sqsubseteq j(y) = y$. This means that $j(x
  \wedge y)$ is a lower bound of $x$ and $y$ so we have $j(x \wedge y) \sqsubseteq x \wedge y$.
...
certain nucleus on frame of downwards-closed subsets of a poset.
