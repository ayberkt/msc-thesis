...
\ref{sec:intr-sys}, \ref{sec:cover}, and \ref{sec:cover-nucleus} correspond respectively,
to modules \modname{FormalTopology}, \modname{Cover}, and \modname{CoverFormsNucleus}.
Sections \ref{sec:cover-nucleus} and \ref{sec:universal-prop} correspond to the
\modname{UniversalProperty} module.
...
\begin{itemize}
  ...
  \item $B$: a type of \emph{experiments} $B(a)$ that one can perform at a certain stage
    of knowledge $a$,
...
makes sense to reserve this terminology to interaction systems with orders satisfying the
...
\begin{defn}[Monotonicity property of an interaction system]\label{defn:mono}
  ...
  with a poset structure $\sqsubseteq$, is said to have the monotonicity property if the following
  type is inhabited:
  \begin{equation*}
...
always an equivalent experiment ($b'$) that will refine our knowledge in an equivalent
way.
...
contained by this relation is sufficient to generate the topology (i.e., the frame). This
generation process amounts to considering all possible joins of these basic opens minus
those that are already joins of some other basic opens as expressed by the cover.
...
\begin{defn}[Naive cover relation]\label{defn:naive-cover}
  ...
  \end{prooftree}
  \]
  To emphasise the reading of this type as a relation, we write the constructors as rule
...
we could restrict this to get an endofunction on the set of downwards-closed subsets:
\begin{equation*}
  \_\RHD\_ : \dcsubset{A} \rightarrow \dcsubset{A}
\end{equation*}
which would then be a nucleus. (\ref{eqn:expected-cover-type}), however, is not the case
so we first have to deal with this problem.
...
\end{equation*}
To prove this, we would need a lemma saying given any $a$, and subsets $U, V$,
\begin{center}
...
principle looking a lot like the axiom of choice~\cite[pg.~119, Lemma~3.8.2]{hottbook}:
\begin{equation*}
...
Coquand~\cite{another-way-out} (privately communicated to the author). We now provide the
revised form, repeating the rules from Defn.~\ref{defn:naive-cover} for the sake of
self-containment.
...
\begin{agdanotation}
  ...
  \veragda{} formalisation and is denoted {\color{AgdaDatatype}\fnname{◀}}.
\end{agdanotation}
...
We now prove some crucial properties of the cover relation.
...
\begin{prop}\label{prop:lem2}
  ...
  $\oftyII{U}{V}{\pow{A_{\McF{}}}}$ be subset of $A_{\McF{}}$ such that $V$ is
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item Case $\rulebranch{}$. We have some experiment $\oftyI{b}{a}$ such that
      ...
      $\mempow{d(a, b, c)}{V}$ so we are done by the inductive hypothesis which gives us
      $\covers{d(a, b, c)}{\intersect{U}{V}}$.
...
\begin{proof}
  ...
  $\covers{a}{V}$. We need to show that $\covers{a'}{\intersect{U}{V}}$. We proceed by
  induction on the proof of $\covers{a'}{U}$.
  \begin{itemize}
    \item Case $\ruledir{}$. We have that $a \in U$. By the downwards-closure of $U$, this
      means that $a' \in U$. The result then follows by Prop.~\ref{prop:lem2}.
...
\begin{enumerate}
  \item Start with a formal topology $\McF{}$ on type $A$.
...
\section{Lifting into the generated frame}
...
$\covers{\_}{\_}$ is a nucleus on this frame (by Theorem~\ref{thm:covering-nucleus}), we
know that the set of fixed points for $\covers{\_}{\_}$ is a frame as well; we denote this
$\fix{\dcframe{P}}{\covernm{}}$. Now, notice that we can define a lifting map
$\eta : \abs{P} \rightarrow \fix{\dcframe{P}}{\covernm{}}$ as follows:
\begin{align*}
  \eta    \quad&:\quad \abs{P} \rightarrow \pow{A_{\McF{}}}\\
  \eta(a) \quad&\is\quad \covers{\_}{(a\downarrow)}
\end{align*}
where $a\downarrow$ denotes the \emph{downwards-closure} of $a$: $\{ a'~|~a' \sqsubseteq a \}$. So $a' \in \eta(a)
\equiv \covers{a'}{a\downarrow}$ is to say ``$a'$ eventually becomes something that $a$ becomes'' if we
take the reading of stages of knowledge being refined over time. As we have proven in
Prop.~\ref{prop:lem1}, $\eta(x)$ is downwards-closed. It is also a fixed point for
$\covernm{}$, as we will show, so its type can be refined to
$\oftyI{\eta}{\abs{P} \rightarrow \fix{\dcframe{P}}{\covernm{}}}$.
...
\begin{defn}[$\eta$]
  ...
  to the underlying poset of $\fix{\dcframe{P}}{\covernm{}}$, defined as:
  \begin{align*}
    \eta    \quad&:\quad \mono{P}{\fix{\dcframe{\abs{P}}}{\_\RHD\_}}\\
...
\abs{\fix{\dcframe{A_{\McF{}}}}{\covernm{}}}$ on the underlying posets. We will define a
...
\bigcup_i V_i$. In the case we are dealing with a $V$ of the form $\setof{ d(a, b, c) ~|~
  \oftyI{c}{C(a, b)} }$, it is clear that the $a \sqsubseteq \bigcup_i V_i$ direction holds meaning by the
monotonicity of $f$: $f(a) \sqsubseteq \bigcup \setof{ f(d(a, b, c)) ~|~ \oftyI{c}{C(a, b)} }$. To assert
$f(a) \sqsubseteq \bigvee_{\oftyI{c}{C(a, b)}} f(d(a, b, c))$ is then to assert that $f$ maps $\McF{}$ to
$R$ as to precisely make the $\covernm{}$ relation hold; hence the reading of this as
``representation''.
...
\begin{figure}
  ...
  \begin{tikzcd}[row sep=40pt, column sep=40pt]\label{fig:comm-diag}
    \McF{} \arrow[swap, rd, "f"] \arrow[r, "\eta"]
      & \fix{\dcframe{A_{\McF{}}}}{\covernm{}} \arrow[d, dashed, "g"] \\
      & R
  \end{tikzcd}
\end{figure}
...
\begin{thm}[Universal property for formal topologies]\label{thm:univ-prop}
  ...
  $\oftyI{g}{\framehomo{\fix{\dcframe{A_{\McF{}}}}{\covernm{}}}{R}}$ such that $f = g \circ
  \eta$, as summarised in Fig.~\ref{fig:comm-diag}

  We express this fully formally as follows. let
  $L \is \fix{\dcframe{A_{\McF{}}}}{\covernm{}}$;
  \begin{align*}
...
\begin{lemma}\label{lem:main}
  ...
  \is \fix{\dcframe{A_{\McF{}}}}{\covernm{}}$. For any $\oftyI{U}{L}$, it is the case that:
  \begin{equation*}
