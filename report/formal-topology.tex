...
One way of reading $\covers{a}{U}$ is as a relaxation of $a \inn U$ to ``it is
eventually the case that $a \inn U$'' if one views the refining of $a$ as an evolution over
time. In other words, it might not be that $a \inn U$ but once stage $a$ has been reached,
...
\begin{proof}
  ...
  $\covers{a}{U}$.
  \begin{itemize}
    \item Case $\ruledir{}$. It must be that $a \inn U$ and hence, by the
      downwards-closure of $U$, $a' \inn U$ meaning $\covers{a'}{U}$ by $\ruledir{}$.
...
\end{prop}
\begin{proof}
  Let $\oftyI{a}{A_{\McF{}}}$ such that $\covers{a}{U}$ and $a \inn V$. We proceed by
...
\begin{proof}
  ...
  proof.
  \begin{itemize}
    \item Case $\ruledir{}$. $a \inn \intersect{U}{V}$ meaning $\mempow{a}{U}$ and
...
\inn \eta(a) \equiv \covers{a'}{\lifta{a}}$ is to say ``$a'$ eventually evolves into a stage
...
\begin{defn}[Representation]\label{defn:rep}
  Given a formal topology $\mathcal{F} \equiv (A, B, C, d)$, a frame $R$, and a function
  $\oftyI{f}{A \rightarrow \abs{R}}$, we say that $f$ represents $\McF$ in $R$ if the
  following type is inhabited:
  \begin{equation*}
    \represents{\McF}{R}{f} \quad\is\quad
      \pity{a}{A}{
        \pity{b}{B(a)}{
          f(a) \sqsubseteq \bigvee^R_{\oftyI{c}{C(a, b)}} f(d(a, b, c)).
        }
      }
  \end{equation*}
\end{defn}

It can be shown that the opposite direction of the inequality expressed in the definition
of $\representsnm$ always holds. To assert $\represents{\McF}{R}{f}$ is then to assert:
\begin{equation*}
  \pity{a}{A}{\pity{b}{B(a)}{f(a) = \bigvee^R_{\oftyI{c}{C(a, b)}} f(d(a, b, c))}}.
\end{equation*}
This property therefore expresses that $R$ behaves like a model of the covering relation
of the formal topology $\McF$, that is, that the join of $R$ reflects the covering
relation induced by the formal-topological structure of $\McF$. This is the idea
underlying the name ``representation''.
...
\begin{proof}
  ...
  $\covers{a}{\bigcup \setof{ \eta(u) ~|~ u \inn U }}$. The result follows by induction on the
...
\begin{lemma}\label{lem:resp-join}
  ...
  $\oftyI{U}{\sub{}{\abs{L}}}$ with index type $I$, if $a \inn \bigvee^L U$ then
  \begin{equation*}
...
\begin{proof}
  ...
  \end{equation*}
  Let $\oftyI{a}{A_{\McF}}$ such that $a \inn \bigvee^L U$. We proceed by induction on the
    ...
    \item Case $\rulebranch{}$. We have some experiment $\oftyI{b}{B(a)}$ such that for
      ...
      \begin{alignat*}{3}
        ...
        \quad&\sqsubseteq\quad   && \bigvee^R \setof{ f(a) ~|~ (\_, (a, \_))
                            : \sigmaty{i}{I}{\sigmaty{a}{A_{\McF{}}}{a \inn U_i}}
                            }
               &&\quad [\text{LUB and IH}].
      \end{alignat*}
...
\begin{proof}[Proof of Theorem~\ref{thm:univ-prop}]
  ...
  \begin{alignat*}{2}
    ...
    g(U) \quad&\is\quad && \bigvee^R \setof{ f(u) ~|~ u \inn U }  .
  \end{alignat*}
  ...
  $\bigvee^R$ is an upper bound of $f(\eta(a))$ so it suffices to show $f(a) \inn f(\eta(a))$.
  ...
  \begin{itemize}
    \item Case: $\ruledir{}$. It must be that $a' \inn \lifta{a}$ which is to say $a'
  ...
  \begin{equation}\label{eqn:pres-meet}
      \bigvee^R \{ f(a) ~|~ a \inn \intersect{U}{V} \}
    = \bigvee^R \setof{ \bigvee^R \{ f(w) ~|~ w \sqsubseteq u \times w \sqsubseteq v \} ~|~ (u, v) \inn U \times V }.
  \end{equation}
  ...
  $\oftyI{a}{A_{\McF{}}}$ such that $a \inn U$ and $a \inn V$. The fact that
  ...
  \begin{equation*}
    \setof{ \bigvee^R \{ f(w) ~|~ w \sqsubseteq u \times w \sqsubseteq v \} ~|~ (u, v) \inn U \times V }.
  \end{equation*}
  ...
  \begin{align*}
    g(\intersect{U}{V}) &\equiv \bigvee^R \{ f(a) ~|~ a \inn \intersect{U}{V} \}
               && [\text{(\ref{eqn:pres-meet})}] \\
             &= \bigvee^R \setof{ \bigvee^R \{ f(w) ~|~ w \sqsubseteq u \times w \sqsubseteq v \} ~|~ (u, v) \inn U \times V }
               && [\text{flatness}]                                                 \\
             &= \bigvee^R \setof{ f(u) \wedge f(v) ~|~ (u, v) \inn U \times V }
               &&\text{[Prop.~\ref{prop:distr}]}                                    \\
             &= \left( \bigvee^R \setof{ f(u) ~|~ u \inn U } \right)
              \wedge \left( \bigvee^R \setof{ f(v) ~|~ v \inn V } \right)                 \\
             &\equiv g(U) \wedge g(V)                                                         .
  \end{align*}
  Let us now show that $g$ preserves joins. Let $U$ be a family of inhabitants of
  $\abs{L}$.
  \begin{align*}
    g(\bigvee^L U) &\equiv \bigvee^R \setof{ f(a) ~|~ a \inn \bigvee^L_i U_i }
                && [\text{Lemma~\ref{lem:resp-join}}]\\
    &= \bigvee^R \setof{ f(a) ~|~ \oftyI{(\_, (a, \_))}{\sigmaty{i}{I}{\sigmaty{a'}{A_{\McF{}}}{a' \inn U_i}}} }
                && [\text{Lemma~\ref{lem:flatten}}] \\
             &= \bigvee^R \setof{ \bigvee^R \img{f}{U_i} ~|~ \oftyI{i}{I} } \\
             &\equiv \bigvee^R \setof{ g(U_i) ~|~ U_i \inn U }.
  \end{align*}
  ...
  \begin{align*}
    g(U) &\equiv \bigvee^R \setof{ f(u) ~|~ u \inn U }
            &&\text{[$f = g' \circ \eta$]} \\
         &= \bigvee^R \setof{ g'(\eta(u)) ~|~ u \inn U }
            &&\text{[$g'$ is a frame homomorphism]} \\
         &= g'\left( \bigvee^L \setof{ \eta(u) ~|~ u \inn U } \right)
