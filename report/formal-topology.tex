...
Sections \ref{sec:lifting} and \ref{sec:universal-prop} correspond to the
...
(but similar ideas are common in the literature, see, for
...
\equiv \covers{a'}{\lifta{a}}$ is to say ``$a'$ eventually evolves into a stage that $a$
...
point for $\covernm{}$, as we will show, so its type can be refined to $\oftyI{\eta}{\abs{P}
  \rightarrow \fix{\dcframe{P}}{\covernm{}}}$.
...
\begin{proof}[Proof of Theorem~\ref{thm:univ-prop}]
  ...
  $\fix{\dcframe{A_{\McF{}}}}{\covernm{}}$. Let $\oftyI{f}{\mono{P}{R}}$ be a \emph{flat}
