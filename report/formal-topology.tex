...
\begin{proof}[Proof sketch]
  ...
  $\rulebranch{}$ case follows by the $\rulebranch{}$ by a direct appeal to the inductive
  hypothesis directly.
\end{proof}
...
\begin{thm}\label{thm:covering-nucleus}
  ...
  $\McF{}$ (defined in Defn.~\ref{defn:cover}) is a nucleus on
  $\dcframe{P}$.
\end{thm}
\begin{proof}
...
take the reading of stages \emph{evolving over time}. One can now see that $\eta(x)$ is
downwards-closed and a fixed point for $\covers{\_}{\_}$, meaning its type can be refined
to $\oftyI{\eta}{A_{\McF{}} \rightarrow \abs{L}}$.
