...
motivation of Petersson and Synek was to generalise $\mathsf{W}$ types to be able to
accommodate mutual induction. Other names for interaction systems are
\emph{tree set constructors} (which is the name that Petersson and
...
makes sense to reserve this terminology to interaction systems that have orderings that
...
expressed as given in the following definition.
...
If this were the case, we could restrict this to get an endofunction on the set of
...
nucleus $\RHD : \dcsubset{A} \rightarrow \dcsubset{A}$. Why exactly is that? To prove the
idempotence law
\begin{equation*}
