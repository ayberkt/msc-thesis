...
\todo{%
  Explain that we will be using the same notations from the HoTT book.
}
...
The structures presented in this thesis will be based on an underlying partially ordered
...
\begin{proof}
  \todo{%
    prove that $\sum$ types actually preserve all h-levels and remark that this is but
    a corollary
  }
\end{proof}
...
\end{prop}
\begin{proof}
  \todo{prove using Hedberg's theorem.}
\end{proof}
...
formalisation in the \modname{JoinSyntax} submodule of the \modname{Frame} module.
\begin{code}%
...
\todo{explain what is looks like in \veragda{}.}
...
\begin{defn}[Propositional truncation]\label{defn:truncation}
  \todo{define as a HIT and write down the induction principle.}
\end{defn}
